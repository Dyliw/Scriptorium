...
class SessionService:
    ...
    def save_session(self, user_id: int, data: Dict[str, Any]) -> SessionResponse:
        if 'id_chapter' not in data or data['id_chapter'] == 0:
            raise HTTPException(
        ...
        if data.get('accuracy', 0) < 0 or data.get('accuracy', 0) > 100:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Accuracy debe estar entre 0 y 100"
            )
        if data.get('total_keystrokes', 0) <= 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        session_data = {
            ...
            "id_mode": data.get('id_mode'),  # id_mode debe venir de la API
        ...
        mode_name = 'classic'
        if session.id_mode:
            mode = self.repository.db.query(ModeSession).filter(
                ModeSession.id_mode == session.id_mode
            ).first()
            if mode:
