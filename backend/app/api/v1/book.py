...
async def list_books(
    ...
    return book_service.list_books(skip=skip, limit=limit, search=search)
...
async def get_chapter(
    chapter_id: int,
    language: str = Query('es', pattern='^(en|es|de)$'), 
    book_service: BookService = Depends(get_book_service)
):
    return book_service.get_chapter(chapter_id, language=language) 

@router.get("/chapters/{chapter_id}/practice", response_model=PracticeContent)
async def get_practice_content(
    chapter_id: int,
    language: str = Query('es', pattern='^(en|es|de)$', description="Idioma para practicar"), 
    book_service: BookService = Depends(get_book_service)
):
