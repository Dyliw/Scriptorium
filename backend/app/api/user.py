...
from app.schemas.user import(
    UserUpdate, UserUpdatePassword, UserProfileResponse, UserPublicProfileResponse, UserPublicResponse, UserResponse
)
...
async def change_my_passqord(
    ...
    return {"message": "Contraseña actualizada existosamente"}

@router.delete("/me")
async def delete_myaccount(
    passwprd:str,
    user_id: int = Depends(get_current_active_user_id),
    user_service: UserService = Depends(get_user_service)
):
    success = user_service.delete_accont(user_id, passwprd)
    if not success: 
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def list_users(
    ...
    return user_service.list_user(skip=skip, limit=limit, search=search)
...
async def delete_avatar(
    user_id: int = Depends(get_current_active_user_id),
    user_service: UserService = Depends(user_id, file)
):
